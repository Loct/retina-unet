...
#Load the original data and return the extracted patches for training/testing
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth,DRIVE_test_scndgroudTruth, Imgs_to_test, patch_height, patch_width):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
    test_masks = load_hdf5(DRIVE_test_groudTruth)
    test_scndmasks = load_hdf5(DRIVE_test_scndgroudTruth)   ## it is already within 0 to 1
    ...
    test_scndmasks = test_scndmasks[0:Imgs_to_test,:,:,:]
    test_imgs = paint_border(test_imgs,patch_height,patch_width)
    test_masks = paint_border(test_masks,patch_height,patch_width)
    test_scndmasks = paint_border(test_scndmasks,patch_height,patch_width)

    data_consistency_check(test_imgs, test_masks,)
    data_consistency_check(test_imgs, test_scndmasks)

    #check masks are within 0-1
    assert(np.max(test_masks)==1  and np.min(test_masks)==0)
    assert(np.max(test_scndmasks)==1  and np.min(test_scndmasks)==0)

    print("\ntest images/masks shape:")
    print(test_imgs.shape)
    print("test images range (min-max): " +str(np.min(test_imgs)) +' - '+str(np.max(test_imgs)))
    print("test masks are within 0-1\n")
    print("test second ground truth are within 0-1\n")

    #extract the TEST patches from the full images
    patches_imgs_test = extract_ordered(test_imgs,patch_height,patch_width)
    patches_masks_test = extract_ordered(test_masks,patch_height,patch_width)
    patches_scndmasks_test = extract_ordered(test_scndmasks,patch_height,patch_width)
    data_consistency_check(patches_imgs_test, patches_masks_test)
    data_consistency_check(patches_imgs_test, patches_scndmasks_test)

    print("\ntest PATCHES images/masks shape:")
    print(patches_imgs_test.shape)
    print("test PATCHES images range (min-max): " +str(np.min(patches_imgs_test)) +' - '+str(np.max(patches_imgs_test)))

    return patches_imgs_test, patches_masks_test, patches_scndmasks_test




# Load the original data and return the extracted patches for testing
# return the ground truth and the second ground truth in its original shape
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, DRIVE_test_scndgroudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
    test_masks = load_hdf5(DRIVE_test_groudTruth)
    test_scndmasks = load_hdf5(DRIVE_test_scndgroudTruth) #already within 0 to 1
    ...
    print("\ntest second ground truth shape:")
    print(test_scndmasks.shape)
    print("test images range (min-max): " +str(np.min(test_imgs)) +' - '+str(np.max(test_imgs)))
    print("test masks are within 0-1\n")
    print("test second ground truth are within 0-1\n")
    ...
    return patches_imgs_test, test_imgs.shape[2], test_imgs.shape[3], test_masks, test_scndmasks
